...
// ============ 地域归一化（数据看板用） ============
// 小程序「粘贴识别地址」早期把省市区整体错位过一级（省里塞市名、市里塞区名，
// 姓名没切净时还会出现"伍哥 深圳市"）。这些订单已经落库，发货地址拼起来是对的、
// 不影响履约，但直接拿来做统计会让省份榜冒出"深圳市""东莞市"。
// 这里在统计入口做一次归一：省份不是省级名称时，尝试认出其中的城市并还原层级。
const PROVINCE_CITIES = {
  ...
  '湖南省': ['长沙','株洲','湘潭','衡阳','邵阳','岳阳','常德','张家界','益阳','郴州','永州','怀化','娄底','吉首'],
...
const CITY_TO_PROVINCE = (function () {
  const m = {};
  Object.keys(PROVINCE_CITIES).forEach((prov) => {
    PROVINCE_CITIES[prov].forEach((c) => { m[c] = prov; });
  });
  return m;
})();
const VALID_PROVINCE_SET = (function () {
  const m = {};
  Object.keys(PROVINCE_CITIES).forEach((p) => { m[p] = true; });
  return m;
})();

// 把一条地址的 省/市/区 归一成正确层级。认不出来就原样返回，不硬猜。
function normalizeRegion(province, city, district) {
  const p = String(province || '').trim();
  const c = String(city || '').trim();
  const d = String(district || '').trim();
  if (!p || VALID_PROVINCE_SET[p]) return { province: p, city: c, district: d };

  // 省份位上其实是个市名（可能还粘着收件人姓名，如"伍哥 深圳市"）：
  // 取最后一段、去掉"市"后查表，命中就把层级整体下移一级还原。
  const tail = p.split(/[\s,，]+/).filter(Boolean).pop() || p;
  const bare = tail.replace(/(市|特别行政区|自治州|地区|盟)$/, '');
  const mapped = CITY_TO_PROVINCE[bare];
  if (mapped) return { province: mapped, city: tail, district: c || d };
...
async function getStats({ period = 'today' }) {
  ...
  for (const o of orders) {
    ...
    // 归一化：修掉历史上省市区错位的订单，否则省份榜会出现"深圳市"这类市级名
    const rg = normalizeRegion(a.province, a.city, a.district);
    provCount[rg.province] = (provCount[rg.province] || 0) + 1;
    geoTotal += 1;
    if (rg.city) {
      const key = `${rg.province} ${rg.city}`;
...
// 图片经 base64 传入，云函数写进云存储。
// ⚠️ HTTP 网关请求体上限实测约 96KB（90KB 通过、100KB 返回 413），
//    base64 还会膨胀 33% → 前端必须把图压到 ~65KB 以内（见 IMAGE_UPLOAD_MAX_BYTES）。
//    wx-server-sdk 没有 getUploadMetadata，无法签发前端直传凭证，只能走中转。
